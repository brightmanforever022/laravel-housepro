...
@section('content')
<input type="hidden" name="temp_date" id="temp_date" value="">
<?php
...
<?php
    $property_date = \App\PropertyDate::where('property_id', $properties[0]->id)->get();
      ...
      $check_dates = \App\BillingInfo::where('booking_id', $properties[0]->booking_id)->first();

      if ($check_dates != null) {
        $check_in = date('d/m/Y', strtotime($check_dates->check_in));
        $check_out = date('d/m/Y', strtotime($check_dates->check_out));
      }

      $host_selected_date = "";
      $property_dates = array();
      if (count($property_date) > 0) { 
            $lastIndex = count($property_date) - 1;
            foreach ($property_date as $key => $date)
            {
            array_push($property_dates, date('d/m/Y', strtotime($date->selected_date)));

              if ($check_in != '' && $check_out != '') { //echo $date->selected_date . '<br>' . $check_in;
                if ($date->selected_date != $check_dates->check_in && $date->selected_date != $check_dates->check_out) { 
                    if ($key == $lastIndex) {
                        $host_selected_date = $host_selected_date.date('d/m/y', strtotime($date->selected_date));
                      }  
                      if ($key != $lastIndex) {
                          $host_selected_date = $host_selected_date.date('d/m/y', strtotime($date->selected_date)).",";
                      }
            ...
            if ($check_in != '' && $check_out != '') {
                array_push($property_dates, $check_in);
                array_push($property_dates, $check_out);
            }

            $sorted_date = $this->helper->sortArrayByDate($property_dates);

            $availabe = '';

            if (count($sorted_date) > 0) { 
                $smallest = date("Y-m-d", strtotime(str_replace('/', '-', $sorted_date[0])));
                end($sorted_date); 
                $key = key($sorted_date);
                $last = str_replace('/', '-', $sorted_date[$key]);
                $largest = date("Y-m-d", strtotime($last));
                $between_dates = $this->helper->getInBetweenDates ($smallest, $largest);

                if (count($between_dates) > 0) {
                    foreach ($between_dates as $dates) {
                        $val = date("d/m/Y", strtotime($dates));
                        if (!in_array($val, $sorted_date)) {
                            $availabe = $dates;
                            break;
                        }
                    }
      ...
      <script>
            var check = '<?php echo $dates; ?>';
            if (check) {
                var availableDates = $.parseJSON('<?php echo $dates; ?>');
            } else{
...
//           if ($host_selected_date) {
//             $disabledates = explode(',', $host_selected_date);
//           } else {
//             $disabledates = false;
//           }
// // dd($disabledates);
//           if ($disabledates) {
//             array_merge($date_list, $disabledates);
//           }

          $lastIndex = count(array_unique($date_list)) - 1;  
          foreach (array_unique($date_list) as $key => $value) {
                if ($key == $lastIndex) { 
...
<script>
        ...
        // {
        // var x = new Array();
        //     datesDisabled = $.parseJSON('<?php //echo $x; ?>');
        // }else 
        // {
        //   datesDisabled = [];
...
{
    // if($( window ).width() > 991)
    $('#my_div').sticky_div({'min_screen_width':991});
...
<input type="hidden" id="selected_date" value="{{ $host_selected_date }}">
</form>
</div>
<!--div class="prev-next">
    <div class="row">
            ...
            <div class="col-md-8">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <br/>
                        <p>Accomodation set by<br/>@if ($properties[0]->user != null){{$properties[0]->user->company}} @endif</p>
            	...
            	<div class="appointment-form">
                	<div class="appointment-form-box" id="my_div">
                        ...
                        {{ csrf_field() }}
                            	...
                            	<div class="col-md-12 ">
                                	<table>
                                    	<tr id="remember_statics">
                                        	<td width="74%" id="total_one">CHF {{ $properties[0]->price_per_night }} x 0 Nights <br></td>
                                            <td width="26%" id="total_two">CHF 0</td>
                                        </tr>
                                        <tr>
                                            <td width="74%" id="total_cleaning_fee">CHF {{ $properties[0]->cleaning_fee }} x 0 Cleaning Fee</td>
                                            <td width="26%" id="amount_cleaning_fee"> CHF 0</td>
                                        </tr>
                                        <tr>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <a href="#" class="add-favorite" data-id="{{ $properties[0]->id }}">
                                        @if (count($favorite)>0)
                                            <i class="fa fa-heart" data-favorite="1"></i><span>REMOVE FROM FAVORITES</span>
                                        @else
                                            <i class="fa fa-heart-o" data-favorite="0"></i><span>ADD TO FAVORITES</span>
                                        @endif
                                    </a>
                                </div>
                            </div>
...
#map_some{
 ...
 $(document).ready(function()
    ...
    $('.add-favorite').on('click', function(e){
        ...
        var obj = $(this).find('i');
        var span_obj = $(this).find('span');
        var is_favorite = $(obj).data('favorite');
        var future_favorite = (is_favorite + 1) % 2;
        console.log(future_favorite);
        $.ajax({
            url: base_url + '/add_favorite/' + property_id,
            type: 'GET',
            dataType: 'text',
            success: function(res){
                console.log(res);
                if(res == 'ok'){
                    $(obj).data('favorite', future_favorite);
                    if(future_favorite == 0){
                        $(obj).addClass('fa-heart-o');
                        $(obj).removeClass('fa-heart');
                        $(span_obj).text('ADD TO FAVORITES');
                    }else{
                        $(obj).addClass('fa-heart');
                        $(obj).removeClass('fa-heart-o');
                        $(span_obj).text('REMOVE FROM FAVORITES');
                    }
                }
            }
        });
